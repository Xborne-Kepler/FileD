...
public class Mva extends LimitedFiringSource {
	...
	public Mva(CompositeEntity container, String name) throws NameDuplicationException, IllegalActionException {
		...
		input = new TypedIOPort(this, "url de fichier", true, false);
		outNc = new TypedIOPort(this, "Nombre_M_de_Client", false, true);
		outK = new TypedIOPort(this, "Nombre de client", false, true);
		output = new TypedIOPort(this, "throughput", false, true);
	...
	public void attributeChanged(Attribute attribute) throws IllegalActionException {
		if (attribute == arrayLength) {
			...
			if (rate < 0) {
				throw new IllegalActionException(this, "Invalid arrayLength: " + rate);
			}
		} else {
	...
	// fonction qui lit le nombre de servers et le nombre de clients
	...
	// Fonction qui lit le service des servers
		...
		try {
			if (file.exists() != true)
				...
				for (int k = 1; k <= N; k++) {
					...
					output.send(0, new DoubleToken(Tvalue[k - 1][Integer.parseInt(attribut) - 1]));
					...
					outNc.send(0, new DoubleToken(Nvalue[k - 1][Integer.parseInt(attribut) - 1]));
